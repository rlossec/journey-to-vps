...
**But :** rejouer toute la chaîne racine → TLD → autoritaire → IP, sans s’arrêter au cache du résolveur.

**Windows :** pas d’équivalent. Enchaîner les 3 étapes suivantes.

## Sortie attendue (VPS)

Les blocs RRSIG / DS sont volontairement coupés (`…`) : le TP vise les **NS** et le **A**.
...
Depuis WSL, `+trace` peut échouer (`no servers could be reached`) : le stub DNS n’autorise pas la chaîne itérative. Dans ce cas, faire la commande **sur le VPS**.
...
| `A 54.36.100.9` | L’autoritaire donne **l’IP**. Fin de chaîne. |
| `timed out` | Un serveur n’a pas répondu à temps ; `dig` réessaie. **≠** domaine cassé. |
